...
# ============= Row-tied (SDEdit) knockoff configuration =============
# T_STAR controls how strongly each knockoff row is tied to its OWN data row.
#   "none"/"uncond"  -> original unconditional sampler; Cov(X, X_tilde) = 0
#   >= num_timesteps -> same as above
#   smaller int      -> stronger row tie, larger Cov(X, X_tilde)
_ts = os.environ.get("T_STAR", "500").strip().lower()
T_STAR = None if _ts in ("none", "", "uncond") else int(_ts)

# Create result directory (new folder; original results are left untouched)
RESULT_DIR = os.environ.get(
    "RESULT_DIR",
    os.path.join(os.path.dirname(__file__), "result_tied",
                 "tstar_%s" % ("uncond" if T_STAR is None else T_STAR)),
)
...
class AutoencoderLinearOut(nn.Module):
    """
    Autoencoder for knockoff generation with a linear decoder output.

    The original construction applies ReLU to the decoder output, which cannot
    represent negative values; against mean-zero standardized covariates this
    forces roughly half of the reconstruction to zero and leaves the residual
    larger than the data itself.  A linear output is the standard choice for
    continuous data and restores a usable reconstruction.
    """
    def __init__(self, input_dim, bottleneck_dim, activation='relu'):
        super().__init__()
        act = nn.ELU() if activation == 'elu' else nn.ReLU()
        self.encoder = nn.Sequential(nn.Linear(input_dim, bottleneck_dim), act)
...
def estimate_n_factors(X, var_explained=0.90, max_frac=0.5):
    """Number of principal components explaining `var_explained` of the variance."""
    X = np.asarray(X, dtype=np.float64)
    Xc = X - X.mean(axis=0, keepdims=True)
    ev = np.linalg.svd(Xc, full_matrices=False, compute_uv=False) ** 2
    if ev.sum() <= 0:
        return 1
    k = int(np.searchsorted(np.cumsum(ev) / ev.sum(), var_explained) + 1)
    return int(max(1, min(k, int(max_frac * X.shape[1]))))


class AutoencoderXKnockoffFixed:
    """
    Autoencoder knockoffs with a linear decoder output and a data-adaptive
    bottleneck.  r_factors=None estimates the latent dimension from the data
    rather than fixing it at one unit.
    """
    def __init__(self, input_dim, r_factors=None, device=None):
        self.input_dim = input_dim; self.r_factors = r_factors
        self.device = device if device is not None else ('cuda' if torch.cuda.is_available() else 'cpu')
        self.autoencoder = None

    def knockoff_construct_autoencoder(self, X, activation='relu', epochs=600, lr=1e-4, verbose=True):
        X = np.asarray(X, dtype=np.float32); n, p = X.shape
        r = self.r_factors if self.r_factors is not None else estimate_n_factors(X)
        self.r_used = r
        self.autoencoder = AutoencoderLinearOut(p, r, activation).to(self.device)
        ...
        return C + sigma * np.random.randn(n, p)
...
class Gaussian:
    ...
    """
    def __init__(self, ridge=1e-6, s_mode="equi", s_cap=1.0):
        # s_cap=1.0   -> CORRECT: s = min(2*lambda_min, 1)   (matches the comment below)
        # s_cap=1e-3  -> reproduces the submitted-paper behaviour (crippled baseline)
        self.ridge = ridge
        self.s_mode = s_mode
        self.s_cap = s_cap
        self.Sigma_ = None
    ...
    def fit(self, X):
        ...
        s_val = min(2.0 * lam_min, self.s_cap)
        if s_val <= 0:
...
class KnockoffGANSubset:
    """
    KnockoffGAN with random subset swaps, following Yoon et al. (ICLR 2019).

    Differs from the full-swap variant in one essential respect: at every step a
    random subset S is drawn and the discriminator is asked to identify WHICH
    coordinates were swapped.  Exchangeability requires swap-invariance for every
    subset, so this trains the generator against the actual condition; a full-swap
    criterion alone is satisfied even by a knockoff independent of X.
    """
    def __init__(self, input_dim, z_dim=64, device=("cuda" if torch.cuda.is_available() else "cpu"),
                 g_hidden=(256, 256), d_hidden=(256, 256), dropout=0.1,
                 lr_g=1e-4, lr_d=1e-4, n_critic=2, lambda_mom=0.5):
        self.input_dim = input_dim; self.z_dim = z_dim; self.device = device
        self.n_critic = n_critic; self.lambda_mom = lambda_mom
        self.G = _MLP(input_dim + z_dim, input_dim, hidden_dims=g_hidden, dropout=dropout).to(device)
        # one logit per coordinate: which entries were swapped?
        self.D = _MLP(2 * input_dim, input_dim, hidden_dims=d_hidden, dropout=dropout).to(device)
        self.optG = optim.Adam(self.G.parameters(), lr=lr_g, betas=(0.5, 0.9))
        self.optD = optim.Adam(self.D.parameters(), lr=lr_d, betas=(0.5, 0.9))
    ...
    @staticmethod
    def _moment_loss(x, x_tilde):
        x0 = x - x.mean(dim=0, keepdim=True); xt0 = x_tilde - x_tilde.mean(dim=0, keepdim=True)
        mean_loss = (x.mean(dim=0) - x_tilde.mean(dim=0)).pow(2).mean()
        cov_x = (x0.T @ x0) / max(x0.shape[0] - 1, 1)
        cov_xt = (xt0.T @ xt0) / max(xt0.shape[0] - 1, 1)
        return mean_loss + (cov_x - cov_xt).pow(2).mean()

    @staticmethod
    def _swap(x, x_tilde, b):
        """Exchange coordinates where b == 1."""
        return x * (1 - b) + x_tilde * b, x_tilde * (1 - b) + x * b

    def train_gan_for_knockoffs(self, X_data, n_epochs=600, batch_size=64, verbose=True):
        X = np.asarray(X_data, dtype=np.float32); n, p = X.shape
        if p != self.input_dim:
            raise ValueError(f"X has p={p}, but initialized with input_dim={self.input_dim}")
        dl = DataLoader(TensorDataset(torch.from_numpy(X)), batch_size=batch_size,
                        shuffle=True, drop_last=True)
        bce = nn.BCEWithLogitsLoss()
        for epoch in range(n_epochs):
            for (xb,) in dl:
                xb = xb.to(self.device); m = xb.shape[0]
                for _ in range(self.n_critic):
                    self.D.train(); self.optD.zero_grad()
                    xt = self.G(torch.cat([xb, self._sample_z(m)], dim=1)).detach()
                    b = (torch.rand(m, p, device=self.device) < 0.5).float()
                    xs, xts = self._swap(xb, xt, b)
                    logits = self.D(torch.cat([xs, xts], dim=1))
                    lossD = bce(logits, b)          # identify the swapped coordinates
                    lossD.backward(); self.optD.step()

                self.G.train(); self.optG.zero_grad()
                xt = self.G(torch.cat([xb, self._sample_z(m)], dim=1))
                b = (torch.rand(m, p, device=self.device) < 0.5).float()
                xs, xts = self._swap(xb, xt, b)
                logits = self.D(torch.cat([xs, xts], dim=1))
                # generator wants the swap to be unidentifiable
                lossG_adv = bce(logits, torch.full_like(b, 0.5))
                lossG = lossG_adv + self.lambda_mom * self._moment_loss(xb, xt)
                lossG.backward(); self.optG.step()
            if verbose and (epoch + 1) % 100 == 0:
                print(f"[KnockoffGAN-subset] Epoch {epoch+1}/{n_epochs} lossG={lossG.item():.4f}")
        return self

    @torch.no_grad()
    def generate_knockoffs(self, X_data, batch_size=512):
        X = np.asarray(X_data, dtype=np.float32); n, p = X.shape
        self.G.eval(); out = np.zeros_like(X, dtype=np.float32)
        for i in range(0, n, batch_size):
            xb = torch.from_numpy(X[i:i+batch_size]).to(self.device)
            out[i:i+batch_size] = self.G(torch.cat([xb, self._sample_z(xb.shape[0])], dim=1)).cpu().numpy()
        return out
...
def knockoff_quality_check():
    ...
    Xk_diff = diff.generate_knockoffs(X_processed, t_star=T_STAR)
...
def run_multiple_simulations_comparison(n_simulations=50):
    ...
    with open(metadata_file, 'w') as f:
        f.write(f"t_star: {T_STAR if T_STAR is not None else 'uncond (original behaviour)'}\n")
        f.write(f"knockoff_sampler: {'row-tied (SDEdit)' if T_STAR is not None else 'unconditional'}\n")
    ...
    for outcome_type in outcome_scenarios:
        ...
        for amp in tqdm(signal_amplitudes, desc=f"Testing {outcome_type}"):
            ...
            for sim in range(n_simulations):
                ...
                Xk_diff = diff_util.generate_knockoffs(X_processed, t_star=T_STAR)
                _XCOV_LOG.append((outcome_type, amp, 'diffusion') + _cross_cov_stats(X_processed, Xk_diff))
                ...
                _XCOV_LOG.append((outcome_type, amp, 'gaussian') + _cross_cov_stats(X_processed, Xk_g))
                ...
                _XCOV_LOG.append((outcome_type, amp, 'gan') + _cross_cov_stats(X_processed, Xk_gan))
                ...
                _XCOV_LOG.append((outcome_type, amp, 'autoencoder') + _cross_cov_stats(X_processed, Xk_ae))
    ...
    # ---- dump the Cov(X, X_tilde) diagnostics requested by Reviewer 1 ----
    xcov_file = os.path.join(RESULT_DIR, 'cross_covariance_diagnostics.txt')
    with open(xcov_file, 'w') as f:
        f.write(f"# Empirical Cov(X, X_tilde) cross-block, t_star={T_STAR}\n")
        f.write("# scenario amplitude generator mean_abs_cov_diag mean_abs_cov_offdiag\n")
        for row in _XCOV_LOG:
            f.write(f"{row[0]} {row[1]:.4f} {row[2]} {row[3]:.6f} {row[4]:.6f}\n")
    print(f"\nCross-covariance diagnostics saved to {xcov_file}")
